...
DATASET_PATH = 'data32/next-day-wildfire-spread'
SAVE_MODEL_PATH = 'savedModels32'

# 👇 tag everything for the 32×32 run
RUN_TAG = "32"
...
def main():
    ...
    print(f'initializing training on single GPU (tag={RUN_TAG})')

    for loss_name in loss_functions:
        print(f"\n====== Training with loss: {loss_name} (tag={RUN_TAG}) ======")
        print("******************************************************************************")
        train(0, args, loss_name)
        print(f"====== Finished training with loss: {loss_name} (tag={RUN_TAG}) ======\n")
        print("******************************************************************************")
...
def train(gpu, args, loss_name):
    ...
    for epoch in range(args.epochs):
        ...
        if validate:
            ...
            if best_f1_score < curr_f1_score:
                ...
                filename_weights = f'model-{model.__class__.__name__}-{loss_name}-bestF1Score-Rank-{rank}-{RUN_TAG}.weights'
                ...
                filename_ckpt = f'model-{model.__class__.__name__}-{loss_name}-bestF1Score-Rank-{rank}-{RUN_TAG}.pt'
                torch.save(checkpoint, os.path.join(SAVE_MODEL_PATH, filename_ckpt))
                print("Checkpoint saved!")
            else:
                print("Model is not being saved")

    # 👇 history files tagged
    pickle.dump(train_loss_history, open(f"{SAVE_MODEL_PATH}/train_loss_history_{RUN_TAG}.pkl", "wb"))
    pickle.dump(val_metrics_history, open(f"{SAVE_MODEL_PATH}/val_metrics_history_{RUN_TAG}.pkl", "wb"))
    ...
    if gpu == 0:
        ...
        model_path = f"{SAVE_MODEL_PATH}/model-{model.__class__.__name__}-{loss_name}-bestF1Score-Rank-{rank}-{RUN_TAG}.weights"
        ...
        # 👇 include tag in model_id and json file name
        model_id = f"{model.__class__.__name__}-{loss_name}-{RUN_TAG}"
        json_file = f"{SAVE_MODEL_PATH}/per_image_metrics_{RUN_TAG}.json"
